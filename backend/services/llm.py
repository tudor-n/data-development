...
class LLMService:
    ...
    def enhance_report(report: QualityReport) -> QualityReport:
        ...
        # OPTIMIZATION: Sort issues by severity (critical first) and only send the top 30 to the LLM
        # to prevent exceeding the context window on extremely messy datasets.
        ...
        # Take only the top 30 issues to send to the LLM
        ...
        # Let the LLM know there were more issues that didn't fit in the prompt
        ...
        try:
            # Using flash for fast response times
...
"""
        try:
            # We use flash because it is fast and has a massive 1M+ token context window for larger CSVs
            model = genai.GenerativeModel("gemini-2.5-flash")
